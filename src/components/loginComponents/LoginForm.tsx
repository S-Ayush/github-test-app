...
function LoginForm({ isSignIn }: { isSignIn: boolean }) {
  ...
  const [CredentialErrorMsg, setCredentialErrMsg] = useState("");
  ...
  const handleSignIn = (userName: string, password: string) => {
    ...
    userApis
      ...
      .then((res) => {
        ...
        window.location.pathname = RouteDefinitions.ROUTE_DASHBOARD;
      })
      .catch((err) => {
        setCredentialErrMsg(err.response.data.err);
  ...
  return (
    <Form onSubmit={formik.handleSubmit}>
      ...
      <LoginError>{!isCredentialValid && CredentialErrorMsg}</LoginError>
    </Form>
  );
}
